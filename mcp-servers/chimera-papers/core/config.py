...
from .platform import get_chimera_root, get_config_path, get_project_root
...
def _legacy_yaml_to_chimera_nested(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    # Legacy oligo_host / oligo_port / oligo keys are obsolete (agent retired); drop them.
    raw.pop("oligo_host", None)
    raw.pop("oligo_port", None)
    raw.pop("oligo", None)
