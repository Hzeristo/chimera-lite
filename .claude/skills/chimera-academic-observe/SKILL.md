...
description: ALWAYS-ACTIVE. The proactive research observer for Chimera Lite — while the Architect analyzes a paper, mechanism, or technical concept, it quietly checks the Obsidian vault for a materially relevant knowledge node and surfaces the connection UNPROMPTED when (and only when) one exists. Uses existing vault tools (obsidian_graph_query, vault_query) — no new MCP tools. Frames connections through the cross-domain-transplantability heuristic ("this connects to X you filed — can it be hijacked for Memory Physics?"). Silent on irrelevant turns. Activate whenever the current turn involves substantive research analysis in Chimera Lite.
---
