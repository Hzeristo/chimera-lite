...
> **REOPENED → REBUILT:** Phase Q — Disciplined Knowledge Extraction — reopened 2026-07-10 (`friction-260710-02`: `extract_paper` had drifted to **atomic-claim dumps**, violating the founding "synthesis → lens → human-readable summary + critique" VISION; the staging gate caught it — none promoted). **Output shape rebuilt 2026-07-10:** `extract_paper` now stages a *reader's* K node — **Synthesis** (BB) + **Lens Critique** (1-2, function-selected — a 2nd only on hybrid detection) + **Attack Vectors** + ARA-disciplined **Mechanism Claims** (the epistemic floor) — with the citation-grounded edges kept. Lens methodology is single-sourced in `prompts/lenses/*.md`, read by BOTH the server extraction and the interactive `chimera-lens-*` skills (`friction-260710-03`). Prior functional seal (`docs/sprints/phase-Q/phase-review.md`, 2026-07-10) is **superseded** by `docs/sprints/phase-Q/Q.R-rebuild.md`. (Phase N truncated 2026-07-09; Phase O sealed 2026-07-08.)
> **Active:** Phase Q (rebuilt, pending re-seal) — 72/72 tests green, ruff clean; the 8 atomic-dump staged nodes were cleared. **Next:** a live re-extraction on a real paper (operator reviews the new node shape) before re-sealing. Also open: `friction-260710-01` (ingest triage — its prompt is already external at `prompts/chimera_sys/reviewer_zero.j2`; the real ask is *tunable* judgement criteria, not relocation), `DEBT-016` (5 missing-markdown papers), `friction-260709-01`'s ambient half.
