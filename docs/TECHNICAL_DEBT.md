...
| DEBT-022 | Phase L.B review (2026-08-12) | `informed_by` is canonical — I0.5 mandates it as the provenance record for AI-informed T/I/D nodes — but not emittable: `StagingService._TYPE_EDGES` implements the pre-r2 seven-edge set. With T/I/D now hand-authored in Obsidian the operator types it by hand, so nothing is blocked, but the invariant has no mechanism. | Med | Add `informed_by` / `collides_with` to the per-type edge sets; see ENFORCEMENT_DEBT D-3. |
| DEBT-023 | Phase L.B review (2026-08-12) | Server-side changes are invisible to a RUNNING MCP server; this hid a verification result three times in one session (agent registry, `stage_deep_read_node` guard, `paper_loader`). No procedure marks a server-side verification provisional until restart. | Med | Fold an explicit MCP-restart step into the seal/verify checklist, or detect module staleness at tool-call time. |
