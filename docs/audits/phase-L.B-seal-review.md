...
|---|---|---|
| Nothing enters `Knowledge/` without `chimera_tier=deep_read` + `ascend_node` | **Held** (repaired) | `_ascend_write` refuses every destination but `Knowledge/` and has exactly one caller; probe: `promote` with tier absent/`scout`/`deep_read` all REFUSED, `ascend`+`deep_read` writes. Was VIOLATED at review start. **Scope annotation (2026-08-12):** this row and its "sole writer" phrasing govern node **creation**. `apply_link_patch` mutates an existing committed node's `graph_edges` with no tier guard and was not examined at this review; the L.C audit surfaced it and the Architect settled it as outside I1.2 — an edge is frontmatter metadata, not committed content (`ENFORCEMENT_DEBT` **D-7**). The verdict stands; the wording was broader than what was verified. |
